...
/**
 * @ClassName: bacthInsertAqiyiNodeAndRelation
 * @Description
 * @Author: hubin
 * @Date: 2020-08-18
 * @Version:v1.0
...
public class BacthInsertAqiyiNodeAndRelation {
    Logger logger = LoggerFactory.getLogger(this.getClass());
    private Session session = null;
    private QueryNaturalPersonsDao queryNaturalPersonsDao;
    private int threadNum;

    public BacthInsertAqiyiNodeAndRelation(Session session, QueryNaturalPersonsDao queryNaturalPersonsDao, int threadNum) {
        this.session = session;
        this.queryNaturalPersonsDao = queryNaturalPersonsDao;
        this.threadNum=threadNum;
    }

    public void bacthInsertAqiyiNode(Map<String, Integer> param) {
        // 自然人源数据提取
        List<InternetAccountEntity> internetAccountList = queryNaturalPersonsDao.QueryAqiyiNode(param);
        if (ObjectUtil.isNotEmpty(internetAccountList)) {
            logger.info("thread [" + threadNum + "] need  synchronize [" + internetAccountList.size() + "] Aqiyi node to neo4j");
            ...
            for (int i = 0; i < internetAccountList.size(); i++) {
                ...
                GroupUtil.group(nodeByLabel, "Aqiyi").add(internetAccountEntity);
            }//组装标签
            ...
            while (var15.hasNext()) {
                ...
                while (var18.hasNext()) {//组装数据
                    ...
                    map.put("aqiyi", StringUtil.StringDeal(n.getAqiyiId()));
...
//                    map.put("serviceNbr", n.getServiceNbr());
                ...
                while (var21.hasNext()) {//分批次插入
                    ...
                    if (!ObjectUtil.isEmpty(batch)) {
                        ...
                        // nodesJoiner.add("MERGE (n{qqId:node.qqId,prdInstId:node.prdInstId,oneidPrdInstId:node.oneidPrdInstId,serviceNbr:node.serviceNbr})");
                        nodesJoiner.add("MERGE (n{aqiyi:node.aqiyi})");
    ...
    public void bacthInsertAqiyiQRel(Map<String, Integer> param) {
        // 用户、自然人关系源数据提取
        List<InternetAccountEntity> userRelList = queryNaturalPersonsDao.QueryAqiyiNaturalRel(param);
        if (ObjectUtil.isNotEmpty(userRelList)) {
            logger.info("thread [" + threadNum + "] need  synchronize [" + userRelList.size() + "] aqiyi_natural Rel to neo4j");
            ...
            for (int i = 0; i < userRelList.size(); i++) {
                ...
                GroupUtil.group(relationByLabel, "aqiyi_natural").add(userRelEntity);
            }
            ...
            while (var15.hasNext()) {
                ...
                while (var8.hasNext()) {
                    ...
                    map.put("aqiyi", StringUtil.StringDeal(relation.getAqiyiId()));
                    relations.add(map);
                }
                ...
                while (var17.hasNext()) {
                    ...
                    relationJoiner.add("MATCH (cc:Aqiyi),(cust:NaturalPerson) WHERE cc.aqiyi = relation.aqiyi AND cust.oneidPrdInstId= relation.oneidPrdInstId");
                    relationJoiner.add("MERGE (cc)-[t:aqiyi_natural{aqiyiId:relation.aqiyi,oneidPrdInstId:relation.oneidPrdInstId,prdInstId:relation.prdInstId,serviceNbr:relation.serviceNbr,latnId:relation.latnId}]->(cust)");//QQ指向自然人  归属自然人
                    Statement statement = new Statement(relationJoiner.toString(), parameter);
                    try {
                        StatementResult aa=this.session.run(statement, TransactionConfig.empty());
                        aa.list();
                    }catch(Exception e){
                        int i=0;
                        while(i < 3){
