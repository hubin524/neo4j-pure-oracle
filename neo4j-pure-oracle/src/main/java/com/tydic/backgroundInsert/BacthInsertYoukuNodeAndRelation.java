...
/**
 * @ClassName: BacthInsertYoukuNodeAndRelation
 * @Description
 * @Author: hubin
 * @Date: 2020-08-18
 * @Version:v1.0
...
public class BacthInsertYoukuNodeAndRelation {
    Logger logger = LoggerFactory.getLogger(this.getClass());
    private Session session = null;
    private QueryNaturalPersonsDao queryNaturalPersonsDao;
    private int threadNum;

    public BacthInsertYoukuNodeAndRelation(Session session, QueryNaturalPersonsDao queryNaturalPersonsDao, int threadNum) {
        this.session = session;
        this.queryNaturalPersonsDao = queryNaturalPersonsDao;
        this.threadNum=threadNum;
    }

    public void bacthInsertYoukuNode(Map<String, Integer> param) {
        // 自然人源数据提取
        List<InternetAccountEntity> internetAccountList = queryNaturalPersonsDao.QueryYoukuNode(param);
        if (ObjectUtil.isNotEmpty(internetAccountList)) {
            logger.info("thread [" + threadNum + "] need  synchronize [" + internetAccountList.size() + "] Youku node to neo4j");
            ...
            for (int i = 0; i < internetAccountList.size(); i++) {
                ...
                GroupUtil.group(nodeByLabel, "Youku").add(internetAccountEntity);
            }//组装标签
            ...
            while (var15.hasNext()) {
                ...
                while (var18.hasNext()) {//组装数据
                    ...
                    map.put("youkuId", StringUtil.StringDeal(n.getYoukuId()));
...
//                    map.put("serviceNbr", n.getServiceNbr());
                ...
                while (var21.hasNext()) {//分批次插入
                    ...
                    if (!ObjectUtil.isEmpty(batch)) {
                        ...
                        // nodesJoiner.add("MERGE (n{YoukuId:node.YoukuId,prdInstId:node.prdInstId,oneidPrdInstId:node.oneidPrdInstId,serviceNbr:node.serviceNbr})");
                        nodesJoiner.add("MERGE (n{youkuId:node.youkuId})");
    ...
    public void bacthInsertYoukuRel(Map<String, Integer> param) {
        // 用户、自然人关系源数据提取
        List<InternetAccountEntity> userRelList = queryNaturalPersonsDao.QueryYoukuNaturalRel(param);
        if (ObjectUtil.isNotEmpty(userRelList)) {
            logger.info("thread [" + threadNum + "] need  synchronize [" + userRelList.size() + "] Youku_natural Rel to neo4j");
            ...
            for (int i = 0; i < userRelList.size(); i++) {
                ...
                GroupUtil.group(relationByLabel, "youku_natural").add(userRelEntity);
            }
            ...
            while (var15.hasNext()) {
                ...
                while (var8.hasNext()) {
                    ...
                    map.put("youkuId",StringUtil.StringDeal( relation.getYoukuId()));
                    relations.add(map);
                }
                ...
                while (var17.hasNext()) {
                    ...
                    relationJoiner.add("MATCH (cc:Youku),(cust:NaturalPerson) WHERE cc.youkuId = relation.youkuId AND cust.oneidPrdInstId= relation.oneidPrdInstId");
                    relationJoiner.add("MERGE (cc)-[t:youku_natural{youkuId:relation.youkuId,oneidPrdInstId:relation.oneidPrdInstId,prdInstId:relation.prdInstId,serviceNbr:relation.serviceNbr,latnId:relation.latnId}]->(cust)");//Youku指向自然人  归属自然人
