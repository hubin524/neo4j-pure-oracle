...
/**
 * @ClassName: BacthInsertQQNodeAndRelation
 * @Description
 * @Author: hubin
 * @Date: 2020-08-18
 * @Version:v1.0
...
public class BacthInsertQQNodeAndRelation {
    Logger logger = LoggerFactory.getLogger(this.getClass());
    private Session session = null;
    private QueryNaturalPersonsDao queryNaturalPersonsDao;
    private int threadNum;

    public BacthInsertQQNodeAndRelation(Session session, QueryNaturalPersonsDao queryNaturalPersonsDao, int threadNum) {
        this.session = session;
        this.queryNaturalPersonsDao = queryNaturalPersonsDao;
        this.threadNum=threadNum;
    }

    public void bacthInsertQQNode(Map<String, Integer> param) {
        // 自然人源数据提取
        List<InternetAccountEntity> internetAccountList = queryNaturalPersonsDao.QueryQqNode(param);
        if (ObjectUtil.isNotEmpty(internetAccountList)) {
            logger.info("thread [" + threadNum + "] need  synchronize [" + internetAccountList.size() + "] QQ node to neo4j");
            ...
            for (int i = 0; i < internetAccountList.size(); i++) {
                ...
                GroupUtil.group(nodeByLabel, "QQ").add(internetAccountEntity);
            }//组装标签
            ...
            while (var15.hasNext()) {
                ...
                while (var18.hasNext()) {//组装数据
                    ...
                    map.put("qqId", StringUtil.StringDeal(n.getQqId()));
...
//                    map.put("serviceNbr", n.getServiceNbr());
                ...
                while (var21.hasNext()) {//分批次插入
                    ...
                    if (!ObjectUtil.isEmpty(batch)) {
                       ...
                       // nodesJoiner.add("MERGE (n{qqId:node.qqId,prdInstId:node.prdInstId,oneidPrdInstId:node.oneidPrdInstId,serviceNbr:node.serviceNbr})");
                        nodesJoiner.add("MERGE (n{qqId:node.qqId})");
    ...
    public void bacthInsertQQRel(Map<String, Integer> param) {
        // 用户、自然人关系源数据提取
        List<InternetAccountEntity> userRelList = queryNaturalPersonsDao.QueryQqNaturalRel(param);
        if (ObjectUtil.isNotEmpty(userRelList)) {
            logger.info("thread [" + threadNum + "] need  synchronize [" + userRelList.size() + "] qq_natural Rel to neo4j");
            ...
            for (int i = 0; i < userRelList.size(); i++) {
                ...
                GroupUtil.group(relationByLabel, "qq_natural").add(userRelEntity);
            }
            ...
            while (var15.hasNext()) {
                ...
                while (var8.hasNext()) {
                    ...
                    map.put("qqId", StringUtil.StringDeal(relation.getQqId()));
                    relations.add(map);
                }
                ...
                while (var17.hasNext()) {
                    ...
                    relationJoiner.add("MATCH (cc:QQ),(cust:NaturalPerson) WHERE cc.qqId = relation.qqId AND cust.oneidPrdInstId= relation.oneidPrdInstId");
                    relationJoiner.add("MERGE (cc)-[t:qq_natural{qqId:relation.qqId,encryptOneidPrdInstId:relation.encryptOneidPrdInstId,oneidPrdInstId:relation.oneidPrdInstId,prdInstId:relation.prdInstId,serviceNbr:relation.serviceNbr,latnId:relation.latnId}]->(cust)");//QQ指向自然人  归属自然人
