...
/**
 * @ClassName: BacthInsertWeiboNodeAndRelation
 * @Description
 * @Author: hubin
 * @Date: 2020-08-18
 * @Version:v1.0
...
public class BacthInsertWeiboNodeAndRelation {
    Logger logger = LoggerFactory.getLogger(this.getClass());
    private Session session = null;
    private QueryNaturalPersonsDao queryNaturalPersonsDao;
    private int threadNum;

    public BacthInsertWeiboNodeAndRelation(Session session, QueryNaturalPersonsDao queryNaturalPersonsDao, int threadNum) {
        this.session = session;
        this.queryNaturalPersonsDao = queryNaturalPersonsDao;
        this.threadNum=threadNum;
    }

    public void bacthInsertWeiboNode(Map<String, Integer> param) {
        // 自然人源数据提取
        List<InternetAccountEntity> internetAccountList = queryNaturalPersonsDao.QueryWeiboNode(param);
        if (ObjectUtil.isNotEmpty(internetAccountList)) {
            logger.info("thread [" + threadNum + "] need  synchronize [" + internetAccountList.size() + "] Weibo node to neo4j");
            ...
            for (int i = 0; i < internetAccountList.size(); i++) {
                ...
                GroupUtil.group(nodeByLabel, "Weibo").add(internetAccountEntity);
            }//组装标签
            ...
            while (var15.hasNext()) {
                ...
                while (var18.hasNext()) {//组装数据
                    ...
                    map.put("weiboId", StringUtil.StringDeal(n.getWeiboId()));
...
//                    map.put("serviceNbr", n.getServiceNbr());
                ...
                while (var21.hasNext()) {//分批次插入
                    ...
                    if (!ObjectUtil.isEmpty(batch)) {
                        ...
                        // nodesJoiner.add("MERGE (n{WeiboId:node.WeiboId,prdInstId:node.prdInstId,oneidPrdInstId:node.oneidPrdInstId,serviceNbr:node.serviceNbr})");
                        nodesJoiner.add("MERGE (n{weiboId:node.weiboId})");
    ...
    public void bacthInsertWeiboRel(Map<String, Integer> param) {
        // 用户、自然人关系源数据提取
        List<InternetAccountEntity> userRelList = queryNaturalPersonsDao.QueryWeiboNaturalRel(param);
        if (ObjectUtil.isNotEmpty(userRelList)) {
            logger.info("thread [" + threadNum + "] need  synchronize [" + userRelList.size() + "] Weibo_natural Rel to neo4j");
            ...
            for (int i = 0; i < userRelList.size(); i++) {
                ...
                GroupUtil.group(relationByLabel, "weibo_natural").add(userRelEntity);
            }
            ...
            while (var15.hasNext()) {
                ...
                while (var8.hasNext()) {
                    ...
                    map.put("weiboId", StringUtil.StringDeal(relation.getWeiboId()));
                    relations.add(map);
                }
                ...
                while (var17.hasNext()) {
                    ...
                    relationJoiner.add("MATCH (cc:Weibo),(cust:NaturalPerson) WHERE cc.weiboId = relation.weiboId AND cust.oneidPrdInstId= relation.oneidPrdInstId");
                    relationJoiner.add("MERGE (cc)-[t:weibo_natural{weiboId:relation.weiboId,oneidPrdInstId:relation.oneidPrdInstId,prdInstId:relation.prdInstId,serviceNbr:relation.serviceNbr,latnId:relation.latnId}]->(cust)");//Weibo指向自然人  归属自然人
